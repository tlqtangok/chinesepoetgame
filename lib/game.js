...
class PoetryGame {
    ...
    selectRandomLines() {
        ...
        const maxIndex = this.poems.length - 2;
        const startIndex = Math.floor(Math.random() * (maxIndex + 1));
        this.currentLines = [this.poems[startIndex], this.poems[startIndex + 1]];
    }
    ...
    createCharCard(char, lineIndex, charIndex) {
        ...
        card.addEventListener('click', () => {
            if (card.textContent) {
                this.speakChar(card.textContent);
            }
        });
    ...
    rebindEvents() {
        ...
        document.querySelectorAll('.drag-char').forEach((charEl, index) => {
            ...
            charEl.addEventListener('click', () => {
                if (!charEl.classList.contains('dragging')) {
                    this.speakChar(charEl.dataset.char);
                }
            });
